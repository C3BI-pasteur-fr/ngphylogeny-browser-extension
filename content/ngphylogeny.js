/*
 * ngphylogeny.js — runs on https://ngphylogeny.fr/workflows/oneclick*
 *
 * If the popup left a pending FASTA, this script pastes it into the
 * "Pasted text" field of the One Click form. It NEVER clicks a submit
 * button: the user checks the form, then starts the analysis themselves.
 */
(async () => {
  'use strict';
  ...
  const CONFIG = {
    ...
    labelPattern: /pasted\s*text/i, // label of the pasted-text field
  ...
  function findTextarea() {
    ...
    const byAttr = all.find((t) =>
      /past|fasta|seq|input|data|text/i.test(`${t.id} ${t.name} ${t.placeholder}`));
  ...
  if (ok) {
    banner(
      `${n} sequence${plural} pasted into "Pasted text". ` +
      'Check the parameters, then start the analysis yourself: the form has not been submitted.',
      { ok: true });
  } else {
    banner(
      `Input field not found on this page. The FASTA (${n} sequence${plural}) ` +
      'can be copied here and pasted manually.',
